    ...
    <div className='p-6'>
        <div className='max-w-6xl mx-auto bg-white p-8 rounded-lg'>
            <div className='flex flex-col md:flex-row'>
                {/* Left Images or Thumbnails*/}
                <div className='hidden md:flex flex-col space-y-4 mr-6 '>
                    {selectedProducts.images.map((image,index)=>{
                        return(
                            <img key={index} src={image.url} onClick={()=>setMainImage(image.url)}
                ...
                <div className='md:w-1/2 md:ml-10'>
                    ...
                    <div className='mb-4'>
                        ...
                        <div className='flex gap-2 mt-2'>
                            {selectedProducts.colors.map((color)=>{
                                return(
                                    <button key={color} className={`w-8 h-8 rounded-full border ${selectedColor===color?"border-black border-4 w-9 h-9":"border-gray-100"}`}
                    ...
                    <div className='mb-4'>
                        ...
                        <div className='flex mt-2 gap-2'>
                            {selectedProducts.sizes.map((size)=>{
                                return <button key={size} onClick={()=>setSelectedSize(size)} className={`px-4 py-2 rounded border ${selectedSize===size?"bg-black text-white":"bg-white"}`}>
                                    {size}
                                </button>
                            })}
                        </div>
                    ...
                    <div className='mb-6'>
                        ...
                        <div className='flex items-center space-x-4 mt-2'>
                            <button onClick={()=>handleQuantity("minus")} className='px-2 py-1 bg-gray-200 rounded text-shadow-lg'>
                                -
                            </button>
                            <span className=' text-lg'>{quantity}</span>
                            <button onClick={()=> handleQuantity("plus")} className='px-2 py-1 bg-gray-200 rounded text-shadow-lg'>
                                +
                            </button>
                        </div>
                    </div>
                    ...
                    <button onClick={handleAddToCart} 
                        ...
                        className={`bg-black text-white py-2 px-6 rounded w-full mb-4 ${isButtonDisabled? "cursor-not-allowed opacity-50":"hover:bg-gray-700"}`}>
